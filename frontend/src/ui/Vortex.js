...
export const Vortex = (props) => {
  ...
  const resize = (canvas, ctx) => {
    ...
    canvas.style.height = `${innerHeight}px`; // Ensure CSS matches canvas size
  ...
  return (
    <div className={cn("fixed inset-0 w-screen h-screen overflow-hidden", props.containerClassName)}>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ref={containerRef}
